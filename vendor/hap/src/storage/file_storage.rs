...
impl Storage for FileStorage {
    ...
    async fn load_pairing(&self, id: &Uuid) -> Result<Pairing> {
        ...
        // In homes with resident hubs, Apple syncs a shared admin controller
        // identity across devices but each device presents Pair-Verify with its
        // own per-device pairing identifier. Only the identifier of the device
        // that ran Pair-Setup is ever stored, so an exact lookup misses for the
        // hubs. Fall back to the admin pairing (same admin key) so their
        // signatures still validate. See `save_pairing`.
    ...
    async fn save_pairing(&mut self, pairing: &Pairing) -> Result<()> {
        // Mirror the most recently saved admin pairing to a stable key so
        // `load_pairing` can fall back to it for controllers whose per-device
        // identifier was never persisted (see `load_pairing`).
