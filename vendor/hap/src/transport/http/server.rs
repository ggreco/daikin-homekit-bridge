    ...
    pub fn run_handle(&self) -> BoxFuture<Result<()>> {
        ...
        async move {
            ...
            loop {
                ...
                let listener_token = event_emitter.lock().await.add_listener(Box::new(move |event| {
                ...
                let event_emitter_cleanup = event_emitter.clone();
                tokio::spawn(async move {
                    http.serve_connection(stream_wrapper, api)
                        .map_err(|e| error!("{:?}", e))
                        .map(|_| ())
                        .await;
                    // Deregister this connection's event listener once the
                    // connection closes; otherwise every (even short-lived or
                    // failed) connection leaks a listener that fires on every
                    // characteristic change for the life of the server.
                    event_emitter_cleanup.lock().await.remove_listener(listener_token);
                });
            }
