// Vendored, near-verbatim copy of `hap` 0.1.0-pre.15 (see this project's
// Cargo.toml `[patch.crates-io]`). We intentionally avoid churning the upstream
// source, so silence its warnings (mostly `deprecated` lints from its older
// `aead`/`chacha20poly1305`/generic-array versions) to keep our build output
// clean. This attribute only affects this crate, not the parent binary.
#![allow(warnings)]
